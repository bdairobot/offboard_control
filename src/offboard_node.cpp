...
class OffboardControl 
{
  ...
  Pose set_pose;
  Pose generated_pose;  /* generated by pose_follow in executCB */
  Pose local_pose;
  ...
  bool pose_follow(const Pose&, const Pose&);
...
void OffboardControl::executeCB(const navigation_3d_msgs::PathNavigationGoalConstPtr & goal)
{
  ...
  Pose current_wp, next_wp;

  moving_state = MOVING_STATE::PAUSE;/* default state as pause */

  next_wp = waypoints[current_number];
  if (!isQuaternionValid(next_wp.pose.orientation)){
      char* message;
      sprintf(message, "Aborting on Pose Num 0 because it was sent with an invalid quaternion");
      ROS_INFO("OFFBOARD: %s", message);
      as_->setAborted(result,message);
      moving_state = MOVING_STATE::PAUSE;
      return;
  } 
  /* calculate original current wp based on the first wp */
  tf::Vector3 here_c, line_a;
  tf::pointMsgToTF(local_pose.pose.position, here_c);
  tf::pointMsgToTF(next_wp.pose.position, line_a);
  tf::Vector3 line_ca = line_a - here_c;
  double yaw_sp = atan2(line_ca.getY(), line_ca.getX());
  current_wp.pose.position = generated_pose.pose.position = local_pose.pose.position;
  tf::quaternionTFToMsg(tf::createQuaternionFromYaw(yaw_sp),generated_pose.pose.orientation);
  ...
  while (!ros::isShuttingDown()) {
    ...
    switch(moving_state) {
      ...
      case MOVING_STATE::FOLLOW_INIT:
        if (yaw_reached(current_wp.pose.orientation)) {
          ROS_INFO("OFFBOARD: Go to first waypoint.");
          moving_state = MOVING_STATE::FOLLOW_POINT;
        } break;

      case MOVING_STATE::FOLLOW_YAW:
        if (yaw_reached(current_wp.pose.orientation)){
          ROS_WARN("OFFBOARD: Reached one waypoint!");
          ...
          if (current_number < wp_number-1){
            ...
            ROS_WARN("OFFBOARD: Finish goal");
            return;
          }
          moving_state = MOVING_STATE::FOLLOW_POINT;
        } break;

      case MOVING_STATE::FOLLOW_POINT: /* go to one waypoint */
        if(pose_follow(current_wp, next_wp)) {
          current_wp  = next_wp;     
          generated_pose.pose.orientation = current_wp.pose.orientation;
          moving_state = MOVING_STATE::FOLLOW_YAW;
        } break;
      default: break;
    }
...
bool OffboardControl::pose_follow(const Pose &current_wp, const Pose &next_wp)
//bool near = cross_sphere_line(pos_s, 1.0f, prev_sp_s, curr_sp_s, pos_sp_s);
{
  ...
  tf::pointMsgToTF(local_pose.pose.position, here_c);
  tf::pointMsgToTF(current_wp.pose.position, line_a);
  tf::pointMsgToTF(next_wp.pose.position, line_b);

  tf::Vector3 line_cb = line_b - here_c;

  if(line_cb.length() < sphere_r){
    generated_pose.pose.position = next_wp.pose.position; /* default as current waypoint */
    if(line_cb.length() < reached_condition) return true; /* reach the point */
    else return false;
  } else {
    ...
    tf::pointTFToMsg(res, generated_pose.pose.position);
    return false;
  }
}
...
void OffboardControl::pubTimerCB(const ros::TimerEvent& event)
{
  ...
  switch (moving_state) {
    ...
    case MOVING_STATE::FOLLOW_INIT:
    case MOVING_STATE::FOLLOW_YAW:
    case MOVING_STATE::FOLLOW_POINT:
